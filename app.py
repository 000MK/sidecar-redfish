...
# from mylib.utils.ServerSentEvent import start_SSE_threading
from load_env import AppPathInitializer, redfish_info
...
class MyNamespaceHelper():
    """不同專案會需要不同的namespace，此類別用來動態添加namespace
    Step1: Define SupportedApiNamespaceNames in redfish_info.yaml
    Step2: Use this class to dynamically add namespaces
    """
    def __init__(self, api: Api, prefix="/redfish/v1"):
        self.api = api
        self.prefix = prefix
        self.proj_name = os.getenv("PROJ_NAME")
        print(f"## [{self.proj_name}] Register namespaces:")

    def register_namespaces(self, mobule_path="mylib.routers.example_router", candidate_ns_var_names=[]):
        # module = importlib.import_module("mylib.routers.Chassis_router")
        # candidate_ns_var_names = ["Chassis_ns", "Chassis_ThermalSubsystem_Fans_ns"]
        module = importlib.import_module(mobule_path)

        for candidate_ns_var_name in candidate_ns_var_names:
            ns = getattr(module, candidate_ns_var_name)
            if ns.name == '':
                print(f"  + [{self.proj_name}] add namespace: {candidate_ns_var_name}({ns.name})")
                self.api.add_namespace(ns, path=self.prefix)
            elif ns.name in redfish_info["SupportedApiNamespaceNames"]:
                print(f"  + [{self.proj_name}] add namespace: {candidate_ns_var_name}({ns.name})")
                self.api.add_namespace(ns, path=self.prefix)
...
# from mylib.routers.Chassis_router import Chassis_ns, Chassis_ThermalSubsystem_Fans_ns
...
from mylib.routers.registries_router import Registries_ns
api.add_namespace(Registries_ns, path="/redfish/v1")
...
def require_auth_on_all_routes():
    ...
    if auth and "username" in auth and "password" in auth:
        ...
        elif status == AuthStatus.AUTHORIZATION_FAILED:
            return Response("Insufficient Privileges", 403)
    if 'X-Auth-Token' in request.headers:
        token = request.headers['X-Auth-Token']
        status = check_session_auth(token)
        if status == AuthStatus.SUCCESS:
            return
        elif status == AuthStatus.AUTHORIZATION_FAILED:
            return Response("Insufficient Privileges", 403)
