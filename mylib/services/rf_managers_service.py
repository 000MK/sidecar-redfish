...
class RfManagersService(BaseService):
    ...
    def save_networkprotocol(self, service: str, setting):
        ...
        """
        if service == "NTP" and setting["ntp_server"] is not None:
    ...
    def get_hostname(self):
        ...
        :return: str, 主機名稱
        """
        return subprocess.run(["/bin/hostname"], capture_output=True, text=True).stdout.strip()
    # =================系統時間===================
    # 取得目前時區 IANA 格式 
    ...
    def get_managers(self, cdu_id):
        ...
        m.TimeZoneName_AllowableValues = redfish_info.get("TimeZoneName@Redfish.AllowableValues")
        return m.to_dict()
    ...
    def patch_ethernetinterfaces(self, ethernet_interfaces_id: str, body: dict):
        ...
        if interface_enabled is not None and ethernet_interfaces_id != "enp1s0f1":
