...
def pytest_addoption(parser):
    ...
    ex: pytest test.py --env=test --proj-name=sidecar-redfish
    """
    print(f"pytest parser addoption: --proj-name")
    parser.addoption("--proj-name", action="store", required=True)
...
def pytest_configure(config):
    """在測試開始前讀取並設置環境變數"""
    proj_name = config.getoption("--proj-name")
    env_filename = config.getoption("--env-file")
    env = config.getoption("--env") # ex: "test-macos"
    env_filepath = None
    if env_filename:
        env_filepath = ProjConstant.build_env_filepath(proj_name, env_filename)
    elif env:
...
class AssertType(str, Enum):
    ...
    VALUE_GT_0   = "value_gt_0" # 陣列的長度要大於0 (很多`Members`會需要這樣測)
    HTTP_STATUS  = "http_status" 

class AssertChecker:
    @classmethod
    def check(cls, assert_type: AssertType, actual_value: Any, expected_value: Any):
        if assert_type in [AssertType.CONSTANT, AssertType.HTTP_STATUS, AssertType.CONFIGURABLE]:
            return actual_value == expected_value
        elif assert_type == AssertType.DYNAMIC:
            return expected_value - 1.5 <= actual_value <= expected_value + 1.5
        elif assert_type == AssertType.VALUE_GT_0:
